...
function changeLanguage() {
    let selectedLanguage = document.getElementById('site_language').value;
    let currentUrl = window.location.href;
    let newUrl = '';

    if(currentUrl.includes('/en') || currentUrl.includes('/az') || currentUrl.includes('/ru') || currentUrl.includes('/tr') || currentUrl.includes('/uz')) {
        newUrl = currentUrl.replace(/\/(en|az|ru|tr|uz)/, '/' + selectedLanguage);
    } else {
...
$(document).ready(function() {
    $('form#contactForm').submit(function(e) {
        ...
        let formData = $(this).serialize() + '&g-recaptcha-response=' + encodeURIComponent(recaptchaResponse);

        $('html, body').animate({
            scrollTop: $("#formFeedback").offset().top
        }, 250);
        ...
        $.ajax({
            ...
            success: function(response) {
                $('#formFeedback').html('<div class="alert alert-success">Message sent successfully!</div>');
                grecaptcha.reset();
            },
            error: function(xhr) {
                ...
                errorMessages += '</div>';
                $('#formFeedback').html(errorMessages);
                grecaptcha.reset();
            }
        });
    });
